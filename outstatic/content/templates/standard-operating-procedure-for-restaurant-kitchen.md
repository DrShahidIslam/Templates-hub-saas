...
**Q: What is the priority if I find an item that has exceeded its shelf life?**
A: Immediately discard the item, log it in the waste tracker sheet (for inventory/cost tracking), and inform the Sous Chef so that prep volumes can be adjusted for the next shift.

**Q: How do we handle a customer allergen request mid-service?**
A: Announce the allergy clearly to the entire line (e.g., "Allergy: Nut on table 4"). All involved staff must wash hands, change gloves, and use cleaned/sanitized equipment to prepare the dish to avoid cross-contact.
