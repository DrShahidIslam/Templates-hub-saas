...
A: A Correction is an immediate fix to remove the non-conformity (e.g., discarding a defective batch). A Corrective Action is a systemic change implemented to prevent that specific error from ever happening again.

**Q: How do I know if a Preventive Action is needed?**
A: Preventive Actions are proactive. They are triggered by data trends, internal audits, management reviews, or risk assessments that suggest a process is trending toward failure, even if a non-conformance has not yet occurred.
