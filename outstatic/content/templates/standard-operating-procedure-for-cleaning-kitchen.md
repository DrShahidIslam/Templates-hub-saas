...
**Q: How often should the sanitizing solution be replaced?**
A: Sanitizing solution should be tested every 2-4 hours using test strips. If the concentration falls below the required ppm (parts per million) or if the solution appears cloudy/dirty, it must be discarded and replaced immediately.

**Q: Is it necessary to dry surfaces after sanitizing?**
A: No. Sanitizers must be allowed to "air dry" on the surface to ensure they remain in contact with the area long enough to effectively kill bacteria (dwell time). Wiping dry with a towel can re-introduce contaminants.
