...
**1. How do I know when it is time for a Clean-In-Place (CIP)?**
A CIP is typically required when the normalized permeate flow drops by 10–15% from the baseline, the salt passage increases by 10%, or the differential pressure across the membrane stage increases by 15%.

**2. Why is my permeate TDS rising?**
Rising TDS often indicates membrane fouling, scaling, or a compromised O-ring/interconnector on the membrane element. Perform a diagnostic test to determine if the issue is localized to one vessel or system-wide.
